...
from src.backend.manager.db_context_manager import DBContextManager
...
class DBChatController:
    ...
    def __init__(self, db: Session):
        ...
        self.context_manager = DBContextManager(db)
    ...
    async def process_message(
        ...
        **kwargs
        ...
        if db_session is None:
            self.context_manager.initialize_session(
    ...
    def get_user_sessions(self, user_id: int, skip: int = 0, limit: int = 100):
        """获取用户的所有会话"""
        return self.context_manager.get_user_sessions(user_id, skip=skip, limit=limit)
