...
class TestChatService:
    ...
    async def test_handle_chat_stream(self, chat_service, chat_request):
        ...
        with (
            patch("src.backend.app.services.chat.stream_chat", _stream),
            patch("src.backend.app.services.chat.retrieve", return_value=[]),
            patch("src.backend.app.services.chat.format_context", return_value=""),
        ):
    ...
    async def test_handle_chat_stream_with_rag_context(
        ...
        with (
            patch("src.backend.app.services.chat.stream_chat", _stream),
            patch(
                "src.backend.app.services.chat.retrieve",
                return_value=[object()],
            ),
            patch(
                "src.backend.app.services.chat.format_context",
